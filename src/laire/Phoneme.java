...
public class Phoneme implements Serializable {
	...
	protected static Set<Character> ensV = new HashSet<Character>(Arrays.asList(' ',' ',' ',' ','E','O',' ',' ','@',' ','e','a',' ','u',' ','A','y','^','o',' ',' ','i'));
	...
	// retourne une liste de de phoneme   partir du tableau de caract  de prononciation
	 public ArrayList <Phoneme> phonetisation(){
	    	phonemes = prononciation.toCharArray(); // on
	    	ArrayList <Phoneme> prononciationMot= new ArrayList <Phoneme>();// phoneme en cr ation
	    	String phonemeCrea = "";
			for (int i=0; i<phonemes.length;i++){ // pour chaque lettre
	    		if (ensC.contains(phonemes[i])){ // si c'est une consonne
	    			phonemeCrea+=phonemes[i]; // on l'ajoute au phoneme en cr ation
	    			prononciationMot.add(new Phoneme(phonemeCrea));// puis on l'ajoute   la liste de phonemes de la pronociation du mot
	    			phonemeCrea = "";
	    				...
	    				if (i==phonemes.length-1){// est si il est le denier du mot
	    	    				prononciationMot.add(new Phoneme(phonemeCrea));// on cr e le phonemes
	    	    				phonemeCrea = "";
	    				}
	    		}
    ...
    // cr e un phoneme inconnu pour si on ne connait pas la prononciation du mot
