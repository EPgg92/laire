...
public class Interface {
	...
	protected JFrame frame;//La fenêtre
	protected JTextField titre;//Zone de texte où l'utilisateur peut entrer le titre de son sonnet
	protected ArrayList <JLabel> labels = new ArrayList<JLabel>();//Une liste de JLabel où des indications sur le sonnet pourront s'afficher
	//protected static Dictionnaire ObjDictionnaire = new Dictionnaire( "FPD.txt" );//Un dictionnaire de la langue française avec phonétisation des mots
	...
	public Interface() {
		...
		frame.setTitle("Lairë ~ Éditeur de sonnets");//Titre de l'interface
		...
		selection.put("Décasyllabes (10 pieds)", 10);
	}

	
	/**
	 * Initialisation du contenu de frame, la fenêtre du programme
	 */
	public void initialize() {//Initialisation de la fenêtre, sa taille, son aspect
		frame = new JFrame();
		...
		/**
		 * Initialisation des JLabels qui indiquent à l'utilisateur quel vers il est en train d'écrire et quelle rime il doit respecter*/
		...
		titre = new JTextField();//La zone où l'utilisateur peut intituler son sonnet
		...
		final JTextPane textPane = new JTextPane();//La zone où l'utilisateur peut écrire son sonnet
		textPane.setFont(new Font("Times New Roman", Font.PLAIN, 14));
		textPane.setBounds(79, 119, 279, 332);
		frame.getContentPane().add(textPane);
		
		JLabel lblPome = new JLabel("Po\u00E8me:");//Un JLabes indiquant où l'utilisateur peut écrire son sonnet
		...
		JTextPane motsProposes = new JTextPane();//La zone de texte contenant les mots proposés à l'utilisateur pour compléter son vers
		...
		JLabel lblMotsProposs = new JLabel("Mots Propos\u00E9s:");//Un Label pour indiquer la fonction du JTextPane motsProposes à l'utilisateur
		...
		JComboBox<String> nbPiedsChoisi = new JComboBox<String>();//Une ComboBox où l'utilisateur peut choisir le nombre de pieds de son sonnet
		...
		nbPiedsChoisi.addItem("Décasyllabes (10 pieds)");
		...
		JTextField versN = new JTextField();//La zone de texte où l'utilisateur indique que vers il aimerait faire compléter
		versN.setBounds(591, 75, 86, 20);
		frame.getContentPane().add(versN);
		versN.setColumns(10);
		
		JLabel lblVersN = new JLabel("Vers n\u00B0:");//Le JLabel indiquant ce qu'est le textField précédent
		...
		btnEnregistrer.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				...
				String titreSonnet = titre.getText();//récupère le titre donné au sonnet
				String fileName = titreSonnet.replaceAll("[/^[^a-zA-Z0-9]*$/]", "");//Enlève les caractère spéciux et les accents
				String textePoeme = textPane.getText();//Récupère le texte du poème
				File poem = new File(fileName + ".txt");//Crée un fichier txt avec le titre modifié d poème
				try{
					FileWriter fw = new FileWriter(poem);
					fw.write(titreSonnet);//écrit le titre original du sonnet dans le fichier txt
					fw.write("\r\n\r\n");//va à la ligne
					fw.write(textePoeme);//écrit le poème dans le fichier txt
					fw.close();
				}catch(Exception ex){
					
				}
				JOptionPane.showMessageDialog(null, "C'est enregistré!");//Popup communiquant à l'utilisateur que son texte est enregistré
			}
		});
		...
		JButton btnCompleterVers = new JButton("Compl\u00E9ter vers");//Bouton compléter vers
		...
		btnCompleterVers.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				...
				String texteEntre = textPane.getText();//Récupère le texte du sonnet
				Sonnet s1 = new Sonnet(texteEntre);//crée un objet contenant ce texte
				String numVersStr = versN.getText();//Récupère le numéro du vers à compléter
				
				try {
					/**Trouve des mots dans le dictionnaires dont le 
					 * phonème final correspond à la rime voulue, et 
					 * dont le nombre de pieds est égal ou inférieur au nombre de pieds
					 * manquants dans le vers à compléter
					*/
				...
				//Sortie des mots proposés et gestion d'erreurs
				Completeur c= new Completeur( piedsManq,  numVers,dictionnaireRime, dictionnaireTypeRime);
				motsProposes.setText(c.getSortie());
				} catch (NumberFormatException e){
					motsProposes.setText("Veuillez entrer le nombre entre 1 et 14 correspondant au vers que vous souhaitez compléter.");
		...
		JButton btnVrifierRimes = new JButton("V\u00E9rifier rimes et pieds");//Bouton vérifier rimes et pieds
		...
		btnVrifierRimes.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				...
				String texteEntre = textPane.getText();//Récupère le texte du sonnet
				Sonnet s1 = new Sonnet(texteEntre);//crée un objet sonnet avec le texte
				...
				while ( i <= ali.size()-1){
					...
					labels.get(i).setText(ali.get(i));//Modifie le texte des labels en conséquent
	...
	/**
	 * Les méthodes suivantes servent à lire les dictionnaire stocker dans les .sav !
	 * 
	 * */
